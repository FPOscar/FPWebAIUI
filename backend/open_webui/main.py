...
"""
    try:
        ...
        print(
            f'Findlay Park AI v{VERSION} - building the best AI user interface.\n'
            'https://github.com/open-webui/open-webui'
        )
...
async def embeddings(request: Request, form_data: dict, user=Depends(get_verified_user)):
    ...
    if not has_model_cache(request):
...
async def chat_completion(
    ...
    try:
        ...
        if not model_item.get('direct', False):
            ...
            missing_base_model = bool(base_model_id and await resolve_request_model(base_model_id) is None)
            ...
            if missing_base_model and ENABLE_CUSTOM_MODEL_FALLBACK:
                ...
                if fallback_model_id:
                    fallback_model = await resolve_request_model(fallback_model_id)
        ...
        try:
            ...
            if metadata.get('assistant_message_id'):
                ctx = await build_chat_response_context(
                    request, form_data, user, model, metadata, tasks, [], request_models
                )
            ...
            if ctx is None:
                ctx = await build_chat_response_context(
                    request, form_data, user, model, metadata, tasks, events, request_models
                )
            else:
