		...
		<div class="flex-1 min-h-0 overflow-y-auto scrollbar-hover pr-1.5">
			<AdminSettingSection title={$i18n.t('Content Extraction')} first>
				...
				{:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'mistral_ocr_azure'}
					<div class="grid grid-cols-1 gap-x-3 gap-y-2.5 sm:grid-cols-2">
						<AdminSettingField
							label={$i18n.t('Endpoint URL')}
							description={$i18n.t('Azure Mistral OCR service endpoint.')}
						>
							<input
								class={inputClass}
						...
						<AdminSettingField
							label={$i18n.t('API Key')}
							description={$i18n.t('API key sent to Azure Mistral OCR.')}
						>
							<SensitiveInput
								variant="settings"
					...
					<AdminSettingField
						...
						description={$i18n.t('Model deployment used by Azure Mistral OCR.')}
					>
						<input
							class={inputClass}
					...
					<AdminSettingRow
						label={$i18n.t('Include Image Base64')}
						description={$i18n.t('Include extracted images as base64 data.')}
						let:labelId
					>
						<Switch
							bind:state={RAGConfig.AZURE_MISTRAL_OCR_INCLUDE_IMAGE_BASE64}
							ariaLabelledbyId={labelId}
						/>
					</AdminSettingRow>
